...
\end{cases} \RR \begin{cases}
	d^2u = d(ydx + xdy) =_\star dydx + dxdy = 2dxdy \\
	d^2v = d(3x^2dx + 2ydy) =_\star 6xdx^2 + 2dy^2
\end{cases}
$$
$\star$ - тут $d^2x = d^2y = 0$

$$
\begin{multline*}
	df = d\phi = \phi'_udu + \phi'_vdv = \phi'_u(ydx + xdy) + \phi'_v(3x^2dx + 2ydy) =\\
	= (y\phi'_u + 3x^2\phi'_v)dx + (x\phi'_u + 2y\phi'_v)dy
\end{multline*}
$$
$$
\begin{multline*}
...
$P_2 = (\frac{\sqrt{3}}{4}, \frac{3}{4}, \frac{1}{2})$ с $\lambda = -2$ 

Как и в предыдущей задаче мы решим эту с помощью Сильвестра. Составим матрицу:
$$
\begin{pmatrix}
